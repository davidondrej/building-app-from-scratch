...
type ClickStat = { source_tag: string; count: number }

export default function Dashboard() {
  const [links, setLinks] = useState<LinkType[]>([])
  const [clicks, setClicks] = useState<Record<string, ClickStat[]>>({})
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="flex items-center gap-4">
              <span className="text-sm font-mono border-2 border-black px-2 py-1 bg-gray-100 hidden sm:block">{user?.email}</span>
              <button
                onClick={logout}
        ...
        <div className="bg-white border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-8 mb-12">
          ...
          <form onSubmit={createLink} className="flex flex-col md:flex-row gap-6 items-end">
            ...
            <div className="md:w-1/3 w-full">
              ...
              <div className="flex">
                <span className="inline-flex items-center px-3 border-2 border-r-0 border-black bg-gray-200 text-black font-bold text-sm">/</span>
                <input
                  type="text"
                  placeholder="SHORT"
                  value={slug}
